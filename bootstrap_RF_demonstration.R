...
for(i in 1:50) {
  # replace = T
    ...
    grf.bsT.err <- mean((as.integer(as.factor(iris[inds,5])) - predict(grf.bsT)$predictions)^2)
  ...
  # replace = F
    ...
    grf.bsF.err <- mean((as.integer(as.factor(iris[inds,5])) - predict(grf.bsF)$predictions)^2)
    
    perf.bs <- bind_rows(perf.bs,
                         data.frame(err = c(rang.bsT$prediction.error,rang.bsF$prediction.error),
...
for(i in 1:50) {
  ...
  grf.bsT.err <- mean((Y[inds] - predict(grf.bsT)$predictions)^2)
  ...
  grf.bsF.err <- mean((Y[inds] - predict(grf.bsF)$predictions)^2)
